...
function parseRobots(content: string) {
  ...
  return { userAgents: [...userAgents], disallow, allow, sitemaps };
}
...
export async function fetchRobots(siteUrl: string): Promise<RobotsData> {
  ...
  if (!result.ok) {
    return { url, exists: false, statusCode: result.statusCode, content: "", userAgents: [], disallow: [], allow: [], sitemaps: [], fetchError: result.error ?? `HTTP ${result.statusCode}` };
  }

  return { url: result.finalUrl, exists: true, statusCode: result.statusCode, content: result.content, ...parseRobots(result.content) };
}
